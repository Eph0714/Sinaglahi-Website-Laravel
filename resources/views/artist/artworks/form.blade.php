...
@section('title', $artwork ? 'Edit Artwork' : 'Add Artwork')

@section('content')

<h1 class="h3 mb-4">{{ $artwork ? 'Edit Artwork' : 'Add Artwork' }}</h1>
...
<form action="{{ $formAction }}" method="post" enctype="multipart/form-data" novalidate>
    ...
    <div class="form-section">
        ...
        <div class="row g-3">
            ...
            <div class="col-md-6" id="customMediumWrap" hidden>
                <label class="form-label">Specify Medium</label>
                <input name="CustomMedium" class="form-control" placeholder="e.g. Egg tempera on wood panel" value="{{ old('CustomMedium', $artwork->CustomMedium ?? '') }}" />
            </div>
            <div class="col-md-6">
    ...
    <button type="submit" class="btn btn-primary-brand btn-lg">Save Artwork</button>
    <a class="btn btn-outline-brand btn-lg" href="{{ route('artist.artworks.index') }}">Cancel</a>
</form>
