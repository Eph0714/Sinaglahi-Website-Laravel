// Cascading Province -> Municipality/City control, reused by every artist
// enrollment/edit form (public registration, the public Join application,
// Super Admin Add/Edit Artist, the artist's own profile editor, and Website
// Settings). Every one of the Philippines' 83 provinces (82 provinces plus
// the 2022 Maguindanao del Norte/del Sur split) has a real municipality/city
// list baked in below, sourced from public PSGC-based data - so Municipality
// is always a genuine dropdown driven by whichever province is selected,
// never a free-text fallback.
//
// Markup contract:
//   <select data-province-select="FieldName">...province options...</select>
//   <div data-municipality-wrap="FieldName" data-field-name="Municipality" data-current-value="Solano"></div>
// The wrap div starts empty - this script creates the actual <select> inside
// it (and recreates it on every province change) so its option list always
// matches the currently selected province.
(function () {
  'use strict';
  ...
  var MUNICIPALITIES_BY_PROVINCE = {
    ...
    "Cotabato": ["Alamada","Aleosan","Antipas","Arakan","Banisilan","Carmen","Kabacan","Kidapawan","Libungan","Magpet","Makilala","Matalam","Midsayap","M'lang","Pigcawayan","Pikit","President Roxas","Tulunan"],
    ...
    "Metro Manila (NCR)": ["Caloocan","Las Piñas","Makati","Malabon","Mandaluyong","Manila","Marikina","Muntinlupa","Navotas","Parañaque","Pasay","Pasig","Pateros","Quezon","San Juan","Taguig","Valenzuela"],
  ...
  function wire(provinceSelect) {
    var fieldKey = provinceSelect.getAttribute('data-province-select');
    var wrap = document.querySelector('[data-municipality-wrap="' + fieldKey + '"]');
    if (!wrap) return;

    var fieldName = wrap.getAttribute('data-field-name');
    var currentValue = wrap.getAttribute('data-current-value') || '';

    function render(preserveCurrent) {
      var province = provinceSelect.value;
      var municipalities = MUNICIPALITIES_BY_PROVINCE[province] || [];
      ...
      placeholder.value = '';
      placeholder.textContent = province ? '-- Select Municipality/City --' : '-- Select a province first --';
      ...
      if (preserveCurrent && currentValue && municipalities.indexOf(currentValue) !== -1) {
    ...
    provinceSelect.addEventListener('change', function () { render(false); });
    render(true);
  }

  document.addEventListener('DOMContentLoaded', function () {
    document.querySelectorAll('[data-province-select]').forEach(wire);
  });
})();
